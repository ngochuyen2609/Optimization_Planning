class ConditionalSum:
	# tính tổng có điều kiện: chỉ cộng trọng số của một số biến thỏa điều kiện cụ thể.
	def __init__(self,x,w,v,name):
		# x: list[VarIntLS]     # Các biến nguyên
		# w: list[int]          # Trọng số tương ứng với từng biến
		# v: int                # Giá trị điều kiện
		self.__x__ = x
		self.__w__ = w
		self.__v__ = v
		self.__name__ = name
	...
	# Tính giá trị ban đầu của tổng điều kiện:
	...
	# Khi giá trị của một biến x thay đổi, cập nhật lại tổng sao cho hiệu quả (không tính lại toàn bộ).
	...
	# Dự đoán sự thay đổi tổng nếu x và y đổi giá trị cho nhau.
	...
	# Dự đoán sự thay đổi giá trị nếu x được gán giá trị v_new.
